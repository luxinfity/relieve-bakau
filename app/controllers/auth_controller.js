...
exports.login = async (req, res, next) => {
    try {
        ...
        if (!user.validateAuth(req.body.password)) return next(exception('Credentials not match', 401));
...
exports.logout = async (req, res, next) => {
    try {
        ...
        if (!user) return next(exception('Not Authorized', 401));
...
exports.refresh = async (req, res, next) => {
    try {
        ...
        if (!user.validateRefresh()) {
            await user.update({ refreshToken: null, tokenValidity: null });
            return next(exception('Refresh Token Expired', 401));
        }
